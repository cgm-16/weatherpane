...
**명시적 전제(Assumptions)**
- 클라이언트는 웹이며, 로컬 영속 저장소로 브라우저 `localStorage`를 사용한다(버전 관리된 JSON 페이로드 방식; 상세는 ‘로컬 저장소 선택 및 구조’ 절 참고). *(정정: 이전 버전은 IndexedDB를 1순위로 명시했으나 실제 구현과 다르다.)*
- 즐겨찾기 목록은 사용자 계정 단위로 서버에 저장되며, 클라이언트는 Bearer Token 등 표준 인증 방식을 사용한다(구체 스킴은 백엔드 선택). *(미구현 — 차기 범위: 서버 계정/인증 없음, 실제로는 기기 로컬에만 저장된다.)*
...
- HTTP 캐싱/조건부 요청(ETag/If-Match/If-None-Match, 304/412 등)은 표준 의미를 따른다. citeturn2search10turn2search3turn2search33turn2search5 *(미구현 — 차기 범위. 원래 설계는 `docs/legacy/favorites-server-sync-design.md` 참고.)*
