...
| UX-12   | 즐겨찾기는 최대 6개까지 저장 가능하다(상한 도달 시 추가 차단, 크래시 없음)                                                           | 스토어 레이어에서 결정성 있게 상한을 강제하기 위함(`AGENTS.md`; `frontend/features/favorites/favorites-store.ts`의 `MAX_FAVORITES = 6`)                                                                                                     |
| UX-13   | 즐겨찾기 순서는 수동으로만 정해지며 영속된다(알파벳/최근순 자동 정렬 없음)                                                           | 사용자가 의도한 배치를 신뢰할 수 있어야 함(`AGENTS.md`)                                                                                                                                                                                     |
| UX-14   | 삭제 취소(undo)는 위치·닉네임을 포함한 정확한 이전 상태를 복원한다(단순 재추가가 아님)                                               | 데이터 손실 방지, 사용자 신뢰(`AGENTS.md`; `frontend/features/favorites/favorites-store.ts`의 `UndoEntry`)                                                                                                                                  |
| UX-15   | 가장 최근 삭제만 취소 가능하며, 새로운 삭제가 발생하거나 5초가 지나면 취소가 만료된다                                                | 취소 상태의 모호함을 방지하는 결정성 있는 범위 설정(`AGENTS.md`의 “가장 최근 삭제만 취소 가능”; 5초 만료는 `docs/skills/favorites-behavior.md`와 `frontend/features/favorites/favorites-store.ts`의 `UNDO_EXPIRY_MS` 및 만료 처리에서 확인) |

> **참고(UX-12~15):** 위 네 항목은 UX-01~11과 달리 이전 대화에서 합의된 결정이 아니라, `AGENTS.md`의 Favorites 제품 규칙과 실제 구현(`frontend/features/favorites/favorites-store.ts`)을 근거로 이번 문서 정합화 작업에서 새로 역기록한 것이다.
...
- 쓰기(`replaceAll(value)`): 배열 전체를 덮어쓴다(`valueRepository.set()`). 추가/삭제/닉네임 변경/정렬 변경 모두 새 배열을 만들어 `replaceAll()`을 호출하며, 부분 업데이트(per-record CRUD)는 없다(`frontend/features/favorites/favorites-store.ts`).
...
> **구현 상태: 미구현 — 차기 범위**(서버가 없으므로 이 절이 설명하는 동기화 타이밍은 적용되지 않는다). 실제로는 닉네임/정렬 변경이 즉시 `replaceAll()`로 로컬 저장소에 반영된다(`frontend/features/favorites/favorites-store.ts`). 원래 설계는 `docs/legacy/favorites-server-sync-design.md` 참고.
