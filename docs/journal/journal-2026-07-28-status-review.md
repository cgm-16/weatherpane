...
리뷰어의 지적대로 이 문서의 주장에는 자동 정합성 검사가 없다. §3의 런타임 결함 4건은 명령과 출력을 함께 남겨 재현 가능하지만, §2와 §4의 문서 대조 주장은 재현 절차가 없어 다음에 누가 읽어도 검증할 수 없다.

**이 문서를 읽는 사람에게:** §3(재현 명령 있음)과 §2·§4(대조 결과만 있음)의 신뢰 수준은 다르다. 후자를 근거로 작업하기 전에 직접 확인하는 편이 낫다.
