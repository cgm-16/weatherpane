...
| `*.webp` 스케치(교차 출처 override는 opaque라 미캐시) | 캐시 우선(cache-first)             |
...
- 안전 가드: `cacheFirst`·`networkFirst` 모두 동일 출처 200 응답만 캐시에
  ...
  rejection을 막는다.
- opaque 캐싱은 뺐다(결정): 한때 `cacheFirst`가 교차 출처 opaque 응답도 캐시했으나,
  opaque 응답은 status가 0이라 교차 출처 404를 성공과 구분할 수 없어 영구 캐시에 넣으면
  안 된다. 향후 원격 매니페스트 override(교차 출처) URL은 매번 새로 받고 오프라인
  캐시에는 넣지 않는다.
