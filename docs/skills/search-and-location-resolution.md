...
- The search input uses a local `inputValue` state for `value=` (updated synchronously on every `onChange`) and a 300 ms debounced call to `updateQuery` (which drives the URL and search results). This decouples the display value from URL state so React never reasserts the old URL value onto the DOM input mid-keystroke — which would interrupt browser IME composition and cause jamo decomposition on Korean input. Do not revert to `value={query}` + immediate `updateQuery` in `onChange`; that is the original bug. Any direct call to `updateQuery` (e.g., Escape) must cancel the pending debounce timer first. SearchPage must consume an internally initiated URL query acknowledgement without overwriting newer local input. Its debounce guard must track external URL invalidation separately from the optimistic internal target, so a render with the still-committed pre-navigation query cannot invalidate newer local input. Any other URL query change is external and authoritative: it cancels a pending debounced update and hydrates inputValue from the URL.
- Escape always cancels pending input before it can update the URL. With an active highlight, Escape restores the displayed query in both inputValue and the URL, superseding any in-flight internal navigation, and clears the highlight; without an active highlight, it clears the query immediately.
- The URL query is authoritative on `/search?q=...`.
- Update the debounce guard only in input/navigation event handlers and the committed URL effect, never during render; a suspended or discarded URL render must not invalidate pending input.
